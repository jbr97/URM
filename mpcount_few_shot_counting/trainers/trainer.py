...
class Trainer(object):
    ...
    def train_epoch(self, model, loss, train_dataloader, val_dataloader, optimizer, scheduler, epoch, best_criterion, best_epoch):
        ...
        for idx,batch in enumerate(train_dataloader):
            train_loss,loss_den,loss_cls,loss_con = self.train_step(model, loss, optimizer, batch, epoch)
            if idx % 100 == 0:
                print(f'Epoch {epoch}: iter {idx}/{len(train_dataloader)} , train_loss {train_loss} ; loss_den {loss_den} ; loss_cls {loss_cls} ; loss_con {loss_con} .')
        ...
        for k, v in additional_meter.avg.items():
            if k=="mse":
                self.log(f'rmse: {torch.sqrt(v):.4f}', end=' ')
            else:
                self.log(f'{k}: {v:.4f}', end=' ')
        self.log(f'best: {best_criterion:.4f}, time: {time.time() - start_time:.4f}')

        self.save_ckpt(model, os.path.join(self.log_dir, f'epoch_{str(epoch)}.pth'))
    ...
    def val(self, model, val_dataloader, checkpoint=None, ):
        self.log('Start val {}'.format(get_current_datetime()))
        ...
        criterion_meter = AverageMeter()
        additional_meter = DictAvgMeter()
        for batch in easy_track(val_dataloader, description=f'Validating...'):
            with torch.no_grad():
                criterion, additional = self.val_step(model, batch)
            ...
            additional_meter.update(additional)
        current_criterion = criterion_meter.avg
        self.log(f'Val criterion: {current_criterion:.4f}', end=' ')
        for k, v in additional_meter.avg.items():
            self.log(f'{k}: {v:.4f}', end=' ')
