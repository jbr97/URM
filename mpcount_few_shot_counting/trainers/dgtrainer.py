...
class DGTrainer(Trainer):
    def __init__(self, log_dir,seed, version, device, log_para, patch_size, mode):
    ...
    def compute_count_loss(self,loss, pred_dmaps, gt_datas, weights=None):
        ...
        elif loss.__class__.__name__ =='ObjectNormalizedL2Loss':
            _, gt_dmaps, _ = gt_datas
            gt_dmaps = gt_dmaps.to(self.device)
            num_objects = gt_dmaps.sum()
            loss_value = loss(pred_dmaps, gt_dmaps, num_objects)
        else:
    ...
    def train_step(self, model, loss, optimizer, batch, epoch):
        ...
        if self.mode == 'simple':
            ...
            optimizer.zero_grad()
            gt_bboxes = gt_datas[0].to(self.device)
            dmaps1, dmaps2, cmaps1, cmaps2, cerrmap, loss_con, loss_err = model.forward_train(imgs1, imgs2,gt_bboxes, gt_cmaps)
            loss_den = self.compute_count_loss(loss, dmaps1, gt_datas) + self.compute_count_loss(loss, dmaps2, gt_datas)
            loss_cls = F.binary_cross_entropy(cmaps1, gt_cmaps) + F.binary_cross_entropy(cmaps2, gt_cmaps)
        ...
        return loss_total.detach().item(),loss_den.detach().item(),loss_cls.detach().item(),loss_con.detach().item()

    def val_step(self, model, batch):
        img, bboxes,dmaps = batch
        img = img.to(self.device)
        bboxes = bboxes.to(self.device)
        dmaps=dmaps.to(self.device)

  
        pred_map,c = model(img,bboxes)
        if dmaps.dim() == 2 and  dmaps.shape[-1] == 1 :
            val_ae = torch.abs(dmaps - pred_map.flatten(1).sum(dim=1)).sum()
            val_se = torch.pow((dmaps - pred_map.flatten(1).sum(dim=1)),2).sum()
            
        else:
            val_ae = torch.abs(dmaps.flatten(1).sum(dim=1) - pred_map.flatten(1).sum(dim=1)).sum()
            val_se = torch.pow((dmaps.flatten(1).sum(dim=1) - pred_map.flatten(1).sum(dim=1)),2).sum()

        return val_ae, {'mae': val_ae,'mse':val_se}
