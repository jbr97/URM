...
class OPEModule(nn.Module):
    ...
    def forward(self, f_e, pos_emb, bboxes):  # [b, 256, 64, 64] [4096, b, 256] [b, 3, 4]
        bs, _, h, w = f_e.size()
        # extract the shape features or objectness
        if not self.zero_shot:
            box_hw = torch.zeros(bboxes.size(0), bboxes.size(1), 2).to(bboxes.device)  # [b, 3, 2]
            box_hw[:, :, 0] = bboxes[:, :, 2] - bboxes[:, :, 0]
            box_hw[:, :, 1] = bboxes[:, :, 3] - bboxes[:, :, 1]
            shape_or_objectness = self.shape_or_objectness(box_hw)  # [b, 3, 2304]
            shape_or_objectness = shape_or_objectness.reshape(
                bs, -1, self.kernel_dim ** 2, self.emb_dim
            )  # [b, 3, 9, 256]
            shape_or_objectness = shape_or_objectness.flatten(1, 2).transpose(0, 1)  # [27, b, 256]
        else:
            shape_or_objectness = self.shape_or_objectness.expand(bs, -1, -1, -1).flatten(1, 2).transpose(0, 1)
        ...
        if not self.zero_shot:
            ...
            appearance = roi_align(
                f_e, boxes=bboxes, output_size=self.kernel_dim,
                spatial_scale=1.0 / self.reduction, aligned=True
            )  # [b*3, 256, 3, 3]
            appearance = appearance.permute(0, 2, 3, 1)
            appearance = appearance.reshape(bs, self.num_objects * self.kernel_dim ** 2, -1)  # [b, 27, 256]
            appearance = appearance.transpose(0, 1)  # [27, b, 256]
        else:
            appearance = None

        query_pos_emb = self.pos_emb(bs, self.kernel_dim, self.kernel_dim, f_e.device)
        query_pos_emb = query_pos_emb.flatten(2).permute(2, 0, 1).repeat(self.num_objects, 1, 1)  # [27, b, 256]

        if self.num_iterative_steps > 0:
            # print(shape_or_objectness.shape, appearance.shape) [27, b, 256] [27, b, 256]
            memory = f_e.flatten(2).permute(2, 0, 1)  # [4096, b, 256]
            all_prototypes = self.iterative_adaptation(shape_or_objectness, appearance, memory, pos_emb, query_pos_emb)
        else:
...
class IterativeAdaptationModule(nn.Module):
    ...
    def forward(self, tgt, appearance, memory, pos_emb, query_pos_emb, tgt_mask=None, memory_mask=None,
...
class IterativeAdaptationLayer(nn.Module):
    ...
    # shape_or_objectness/tgt, appearance, memory/f_e
    def forward(self, tgt, appearance, memory, pos_emb, query_pos_emb, tgt_mask, memory_mask,
                tgt_key_padding_mask, memory_key_padding_mask):
        if self.norm_first:
            if not self.zero_shot:
